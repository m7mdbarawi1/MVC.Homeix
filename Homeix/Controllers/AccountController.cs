...
namespace Homeix.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(string email, string password, string? returnUrl = null)
        {
            ...
            var claims = new List<Claim> { new Claim(ClaimTypes.Name, user.FullName), new Claim(ClaimTypes.Email, user.Email),new Claim(ClaimTypes.Role, user.Role.RoleName),new Claim("UserId", user.UserId.ToString())};
            await HttpContext.SignInAsync("HomeixAuth",new ClaimsPrincipal(new ClaimsIdentity(claims, "HomeixAuth")), new AuthenticationProperties { IsPersistent = true });
            if (!string.IsNullOrWhiteSpace(returnUrl) && Url.IsLocalUrl(returnUrl)) return Redirect(returnUrl);
            return user.Role.RoleName switch {"admin" => RedirectToAction("AdminDashboard", "Dashboard"),"customer" => RedirectToAction("CustomerDashboard", "Dashboard"),"worker" => RedirectToAction("WorkerDashboard", "Dashboard"), _ => RedirectToAction("Index", "Home")
            };
        }
        ...
        public async Task<IActionResult> MyProfile(User model, string? NewPassword, IFormFile? ProfileImage)
        {
            ...
            if (!string.IsNullOrWhiteSpace(NewPassword)) dbUser.PasswordHash = HashPassword(NewPassword);
            ...
            var claims = new List<Claim> {new Claim(ClaimTypes.Name, dbUser.FullName), new Claim(ClaimTypes.Email, dbUser.Email), new Claim(ClaimTypes.Role, role!.RoleName),new Claim("UserId", dbUser.UserId.ToString())};
            await HttpContext.SignInAsync("HomeixAuth",new ClaimsPrincipal(new ClaimsIdentity(claims, "HomeixAuth")), new AuthenticationProperties { IsPersistent = true });
        ...
        public IActionResult HomeRedirect()
        {
            ...
            {
                if (User.IsInRole("admin")) return RedirectToAction("AdminDashboard", "Dashboard");
                if (User.IsInRole("customer")) return RedirectToAction("CustomerDashboard", "Dashboard");
                if (User.IsInRole("worker")) return RedirectToAction("WorkerDashboard", "Dashboard");
            }
        ...
        private bool VerifyPassword(string password, string storedHash)
        {
            ...
            if (!(storedHash.StartsWith("$2a$") || storedHash.StartsWith("$2b$") || storedHash.StartsWith("$2y$"))) return false;
        ...
        public async Task<IActionResult> ForgotPassword(string email)
        {
            ...
            try
            {
                var smtp = new SmtpClient("smtp.gmail.com"){Port = 587, EnableSsl = true, Credentials = new NetworkCredential("homeix.jo@gmail.com","hzvtxpummkppegkv")};
