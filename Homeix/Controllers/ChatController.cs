...
namespace Homeix.Controllers
{
    ...
    public class ChatController : Controller
    {
        ...
        public async Task<IActionResult> Index(int? userId)
        {
            ...
            var conversation = await _context.Conversations
                .FirstOrDefaultAsync(c =>
                    (c.User1Id == currentUserId && c.User2Id == userId.Value) ||
                    (c.User1Id == userId.Value && c.User2Id == currentUserId));
        ...
        public async Task<IActionResult> Send(int conversationId, string messageText)
        {
            ...
            // Send consistent payload (message, not text)
            await _hubContext.Clients.Groups($"user-{currentUserId}", $"user-{otherUserId}")
